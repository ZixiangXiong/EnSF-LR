...
def main(seed):
    ...
    for k in range(0, T):
        ...
        if k == T - 1:
            pd.DataFrame(x_0).to_csv("x_0_traj_linear_EnKF_LR_lorenz63_{}%obs_{}obsgap_seed{}.csv".format(int(p * 100.), obs_gap,seed_kf))
            pd.DataFrame(x_1).to_csv("x_1_traj_linear_EnKF_LR_lorenz63_{}%obs_{}obsgap_seed{}.csv".format(int(p * 100.), obs_gap,seed_kf))
            pd.DataFrame(x_2).to_csv("x_2_traj_linear_EnKF_LR_lorenz63_{}%obs_{}obsgap_seed{}.csv".format(int(p * 100.), obs_gap,seed_kf))
            pd.DataFrame(x_0_forecast).to_csv("x_0_traj_linear_forecast_EnKF_LR_lorenz63_{}%obs_{}obsgap_seed{}.csv".format(int(p * 100.), obs_gap,seed_kf))
            pd.DataFrame(x_1_forecast).to_csv("x_1_traj_linear_forecast_EnKF_LR_lorenz63_{}%obs_{}obsgap_seed{}.csv".format(int(p * 100.), obs_gap,seed_kf))
            pd.DataFrame(x_2_forecast).to_csv("x_2_traj_linear_forecast_EnKF_LR_lorenz63_{}%obs_{}obsgap_seed{}.csv".format(int(p * 100.), obs_gap,seed_kf))
