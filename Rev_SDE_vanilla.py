...
class REVERSE_SDE2:
    def __init__(self, pseudo_time_step, prior_ensemble, ensemble_size, obs, sigma, y_dim, indxob, indx_indxob_linear):
        ...
        ### Additionally, we assign linear operator to "x_0","x_4" and nonlinear operator to "x_3","x_5"
    ...
    def score_likelihood(self, xt, t, indx_indxob_linear, indx_indxob_nonlinear):
        ...
        #score_x[:, indx_indxob_nonlinear] = (-(xt[:, indx_indxob_nonlinear] ** 3 - self.obs[indx_indxob_nonlinear]) / self.obs_sigma[indx_indxob_nonlinear] ** 2) * (3.0 * xt[:, indx_indxob_nonlinear]**2)
        tau = self.g_tau(t)
        return tau * score_x
    ...
    def reverse_SDE(self):
        ...
        for i in range(self.p_time_step):
            ...
            alpha_t = self.cond_alpha(t)
            sigma2_t = torch.tensor(self.cond_sigma_sq(t))
            diffuse = self.g(t)
            drift_fun = self.f(t)
