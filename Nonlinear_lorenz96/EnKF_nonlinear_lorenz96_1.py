...
def main(seed):
    ...
    for k in range(0, T):
        ...
        if k == T - 1:
            pd.DataFrame(x_0).to_csv("x_0_traj_EnKF_nonlinear_LR_{}%obs_{}obsgap_seed{}.csv".format(int(p * 100.), obs_gap, seed_kf))
            pd.DataFrame(x_1).to_csv("x_1_traj_EnKF_nonlinear_LR_{}%obs_{}obsgap_seed{}.csv".format(int(p * 100.), obs_gap, seed_kf))
            pd.DataFrame(x_0_forecast).to_csv("x_0_traj_forecast_EnKF_nonlinear_LR_{}%obs_{}obsgap_seed{}.csv".format(int(p * 100.), obs_gap,seed_kf))
            pd.DataFrame(x_1_forecast).to_csv("x_1_traj_forecast_EnKF_nonlinear_LR_{}%obs_{}obsgap_seed{}.csv".format(int(p * 100.), obs_gap,seed_kf))
    ...
    if linear_regression == 0:
        pd.DataFrame(errora_k, columns=["RMSE_enkf"]).to_csv(
            "RMSE_EnKF_nonlinear_{}%obs_{}obsgap_seed{}.csv".format(int(p * 100.), obs_gap, seed_kf))
        if p < 1:
            pd.DataFrame(errora_obs, columns=["RMSE_obs"]).to_csv(
                "RMSE_EnKF_nonlinear_observed_{}%obs_{}obsgap_seed{}.csv".format(int(p * 100.), obs_gap, seed_kf))
            pd.DataFrame(errora_unobs, columns=["RMSE_unobs"]).to_csv(
                "RMSE_EnKF_nonlinear_unobserved_{}%obs_{}obsgap_seed{}.csv".format(int(p * 100.), obs_gap, seed_kf))
    elif linear_regression == 1:
        pd.DataFrame(errora_k, columns=["RMSE_enkf"]).to_csv(
            "RMSE_EnKF_nonlinear_LR_{}%obs_{}obsgap_seed{}.csv".format(int(p * 100.), obs_gap, seed_kf))
        if p < 1:
            pd.DataFrame(errora_obs, columns=["RMSE_obs"]).to_csv(
                "RMSE_EnKF_nonlinear_LR_observed_{}%obs_{}obsgap_seed{}.csv".format(int(p * 100.), obs_gap,seed_kf))
            pd.DataFrame(errora_unobs, columns=["RMSE_unobs"]).to_csv(
                "RMSE_EnKF_nonlinear_LR_unobserved_{}%obs_{}obsgap_seed{}.csv".format(int(p * 100.), obs_gap,seed_kf))
