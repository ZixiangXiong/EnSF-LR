...
class REVERSE_SDE:
    def __init__(self, pseudo_time_step, prior_ensemble, ensemble_size, obs, sigma, y_dim, indxob, indx_indxob_linear, initial_std):
        ...
        self.prior_ensemble = np.copy(prior_ensemble)
        self.eps_alpha = 0.05
        self.eps_beta = 0.025
        ...
        self.scalefact = 1.                      ### scalefact is a constant in pyhscis.
        self.indxob = indxob
    ...
    def normalize(self, indx_indxob_linear, indx_indxob_nonlinear):
        ...
        self.obs_sigma[indx_indxob_nonlinear] = np.where(abs(self.obs[indx_indxob_nonlinear]) < np.arctan(100),(self.obs_sigma[indx_indxob_nonlinear] * 1.),self.obs_sigma[indx_indxob_nonlinear] * 1e4)
    ...
    def reverse_SDE(self):
        ...
        xt_array = np.zeros((int(0.02*self.p_time_step), self.y_dim))
        #x_save = np.zeros((1000,3))
        #y_save = np.copy(self.tan_y)
        t = 1.0
        tolerance = 0.01
        step0 = 2000
        ...
        for i in range(self.p_time_step):
            ...
            #x_save[i,:] = mean_X0 + xt_temp.numpy().mean(axis=0) * std_X0
            #x_save[i, :] = xt_temp.numpy().mean(axis=0)
            if np.linalg.norm(xt_temp.mean(dim=0).numpy()-xt_array.mean(axis=0), ord=np.inf) < tolerance and i > 0.02 * self.p_time_step:
                xt = xt_temp
            ...
            """
            if self.step < step0:
                if i > 1. * self.p_time_step:
                    print("Pseudo time stops at ", i)
                    break
            elif self.step == step0:
                if i > 1. * self.p_time_step:
                    print("Pseudo time stops at ", i)
                    break
            """
        ...
        #pd.DataFrame(x_save).to_csv("x_save_lorenz63.csv")
        #pd.DataFrame(y_save).to_csv("y_save_obs_lorenz63.csv")
        #x_ens_analysis_new = xt.numpy()
