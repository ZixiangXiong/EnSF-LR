    ...
    for k in range(0, T):
        ...
        if k % obs_gap == 0:
            ...
            if linear_regression == 1 and Ny < Nx:
                ...
                x_ens_analysis = user.reverse_SDE()
                Xa_k = x_ens_analysis.copy().T
                print("obs_analysis error pre=",np.sqrt(((Xa_k.mean(axis=1)[obs_comp] - x_truth[k, obs_comp]) ** 2).mean()))
                print("unobs_analysis error pre=",np.sqrt(((Xa_k.mean(axis=1)[unobs_comp] - x_truth[k, unobs_comp]) ** 2).mean()))
