...
for k in range(0, T):
    ...
    if k % obs_gap == 0:
        ...
        #print("x_ens_analysis", x_ens_analysis.mean(axis=0))
...
if linear_regression == 0:
    df.to_csv("RMSE_hristo_EnSF_nonlinear_{}%obs_{}obsgap.csv".format(int(p*100.), obs_gap))
    if p < 1:
        pd.DataFrame(errora_obs, columns=["RMSE_obs"]).to_csv("RMSE_hristo_EnSF_nonlinear_observed_{}%obs_{}obsgap.csv".format(int(p * 100.), obs_gap))
        pd.DataFrame(errora_unobs, columns=["RMSE_unobs"]).to_csv("RMSE_hristo_EnSF_nonlinear_unobserved_{}%obs_{}obsgap.csv".format(int(p * 100.), obs_gap))
elif linear_regression == 1:
    df.to_csv("RMSE_hristo_EnSF_nonlinear_LR_{}%obs_{}obsgap.csv".format(int(p * 100.), obs_gap))
    if p < 1:
        pd.DataFrame(errora_obs, columns=["RMSE_obs"]).to_csv("RMSE_hristo_EnSF_nonlinear_LR_observed_{}%obs_{}obsgap.csv".format(int(p * 100.), obs_gap))
        pd.DataFrame(errora_unobs, columns=["RMSE_unobs"]).to_csv("RMSE_hristo_EnSF_nonlinear_LR_unobserved_{}%obs_{}obsgap.csv".format(int(p * 100.), obs_gap))
