...
for k in range(0, T):
    ...
    if k % obs_gap == 0:
        ...
        if linear_regression == 0:
            ...
            #print("Xa_k", Xa_k[0:10, :].mean(axis=1), Xa_k.shape)
            ...
            Yobs_k = np.outer(y_k, np.ones(Ne)) + Eobs_k;  # [Ny,Ne]
            # Scaled innovation matrix
            Pf_k_sub = np.cov(H_k @ Xf_k);  # [Ny,Ny]
            D_k = Yobs_k - H_k @ Xf_k;  # [Ny,Ne]
            IN_k = R_k + Pf_k_sub;  # [Ny,Ny]
            Z_k = np.linalg.solve(IN_k, D_k);  # this gives (IN)^-1 * D =: Z [Ny,Ne]
            # EnKF update, only observed states are updated
            Xa_k[obs_comp, :] = Xf_k[obs_comp, :] + Pf_k_sub @ Z_k
...
if linear_regression == 0:
    df.to_csv("RMSE_hristo_EnKF_linear_{}%obs_{}obsgap.csv".format(int(p*100.), obs_gap))
elif linear_regression == 1:
    df.to_csv("RMSE_hristo_EnKF_linear_LR_{}%obs_{}obsgap.csv".format(int(p * 100.), obs_gap))
